...
if (Input::exists()) {
    ...
    if($settings->recaptcha == 1){
        ...
        if ($response != null && $response->success) {
            ...
            $reCaptchaValid=FALSE;
            $error_message .= 'Por favor verifique el reCaptcha.';
        }
    }else{
        $reCaptchaValid=TRUE;
    }
    ...
    if($reCaptchaValid || $settings->recaptcha == 0){ //if recaptcha valid or recaptcha disabled
        ...
        if ($validation->passed()) {
            ...
            if ($login) {
                ...
                $error_message .= 'Falló el inicio de sesión. Por favor verifique su nombre de usuario y contraseña e inténtelo otra vez.';
            }
        } else{
            $error_message .= '<ul>';
...
?>
    ...
    <div class="form-group">
        ...
        <input  class="form-control" type="text" name="username" id="username" placeholder="Nombre de usuario/Email" required autofocus>
    </div>
    ...
    ?>
    <div class="form-group">
    <label> Por favor marque el tilde para continuar</label>
